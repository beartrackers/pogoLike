...
function Splash(posGrid,pos,color) 
{
	this.name = "Splash";
	this.enabled = true;
	this.started = false;
	...
	this.velocity = 10;
	this.color = color;
	this.gridPos = posGrid;
	this.Transform = {};
	this.Transform.RelativePosition = pos;
	...
	this.Start = function() 
	{
		if (!this.started) {
			// operation start
			this.SetPosition()
			this.SetSize(Application.LoadedScene.grid.caseLength,Application.LoadedScene.grid.caseLength);
			this.SetPivot(.5,.5);
			this.SetScale(.1,.1);
	...
	this.Update = function() 
	{
		...
		if(this.Transform.Scale.x < 1){
			...
			Application.LoadedScene.grid.Tiles[IndexFromCoord(this.gridPos.x,this.gridPos.y, Application.LoadedScene.grid.cases)] = this.color;
			this.isDraw = true;

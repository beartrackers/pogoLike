...
function Player2() 
{
	...
	this.Update = function() 
	{
		if(this.canMove){
			// Left
			if (Input.KeysDown[81] && this.gridPos.x > 0) {
				this.gridPos.x --;
				this.Transform.RelativePosition.x -= this.grid.caseLength;		
				this.canMove = false;
			} 
			// Top
			else if (Input.KeysDown[90] && this.gridPos.y > 0) {
				this.gridPos.y --;
				this.Transform.RelativePosition.y -= this.grid.caseLength;
				this.canMove = false;
			}
			// Right
			else if (Input.KeysDown[68] && this.gridPos.x < this.grid.cases-1) {
				this.gridPos.x ++;
				this.Transform.RelativePosition.x += this.grid.caseLength;
				this.canMove = false;
			}
			// Both
			else if (Input.KeysDown[83] && this.gridPos.y < this.grid.cases-1) {
				this.gridPos.y ++;
				this.Transform.RelativePosition.y += this.grid.caseLength;
				this.canMove = false;
			} 
